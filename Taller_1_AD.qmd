...
subtitle: "Análisis del uso de la energía en el hogar"
format: html
editor: visual
format:
  html:
...
\begin{itemize}
  ...
  \item Coste mensual promedio de la factura energética en euros.
  \item Gasto promedio en litros de combustibles fósiles en euros.
...
\begin{itemize}
  ...
  \item Cual es la percepción sobre la relevancia del cambio climático: "Nada importante", "Poco importante", "Neutral", "Importante" y "Muy importante". -
  \item Con que frecuencia se realizan prácticas de ahorro energético en el hogar: "Nunca", "Rara vez", "A veces", "Frecuentemente" y "Siempre".
\end{itemize}
```
...
\begin{itemize}
  ...
  \item Zona de residencia: "Urbana", "Interurbano" o "Rural".
  \item Existencia de fuentes de energía renovables en el hogar: "Sí", "No".
\end{itemize}
```
...
Donde postularé que la variable $Y$, que referencia al consumo energético de un hogar, tiene una relación lineal con el coste de la factura energética $X_1$ y la superfície del hogar $X_2$.
...
```{r}
chisq.test(estadistico)
```

El *p-valor* no es significativo, por tanto no tenemos suficiente evidencia como para rechazar que ambas medias sean iguales.
...
Por otro lado, no hemos podido extraer ninguna conclusión interesante de la relación entre las variables que hemos estudiado, ya que no parece haber una relación entre estas.
